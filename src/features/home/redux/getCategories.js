...
export function getCategories(args = {}) {
  return (dispatch) => { // optionally you can have getState as the second argument
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const doRequest = axios.get('http://'+VALUES.BD_ORIGIN+':3000/categories/')
      doRequest.then(
        (res) => {
          dispatch({
            type: HOME_GET_CATEGORIES_SUCCESS,
            data: res,
          });
          resolve(res);
        },
        ...
        (err) => {
          dispatch({
            type: HOME_GET_CATEGORIES_FAILURE,
            data: { error: err },
          });
          reject(err);
        },
...
export function reducer(state, action) {
  switch (action.type) {
    case HOME_GET_CATEGORIES_BEGIN:
    ...
    case HOME_GET_CATEGORIES_SUCCESS:
    ...
    case HOME_GET_CATEGORIES_FAILURE:
    ...
    case HOME_GET_CATEGORIES_DISMISS_ERROR:
