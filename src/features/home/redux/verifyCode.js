...
export function verifyCode(args = {}) {
  return dispatch => {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const doRequest = axios.post('http://' + VALUES.BD_ORIGIN + ':3000/verifycode', args);
      doRequest.then(
        res => {
          dispatch({
            type: HOME_VERIFY_CODE_SUCCESS,
            data: res,
          });
          resolve(res);
        },
...
export function reducer(state, action) {
  switch (action.type) {
    ...
    case HOME_VERIFY_CODE_SUCCESS:
      // The request is success
      return {
        ...state,
        verifyCodePending: false,
        verifyCodeError: null,
        codeverified: action.data,
      };

    case HOME_VERIFY_CODE_FAILURE:
    ...
    case HOME_VERIFY_CODE_DISMISS_ERROR:
